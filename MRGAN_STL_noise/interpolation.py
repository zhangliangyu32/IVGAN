...
ndf = 64 
nef = 16
itfr_sigma = {0: 0.05, 20: 0.01, 50: 1e-3, 100: 0}
...
parser.add_argument('--dataroot', default='~/datasets/data_stl10', help='path to dataset')
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    dataset = dset.STL10(root=opt.dataroot, split='unlabeled',
                        transform=transforms.Compose([
                            transforms.Resize(imageSize),
                            transforms.CenterCrop(imageSize),
                            transforms.ToTensor(),
...
for epoch in range(nepochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        output = netD(real + sigma * pixd_noise)
        ...
        real_reconstruted = netG(netE(real))
        reg = lam1 * criterion_reconstruct(real, real_reconstruted) - lam2 * criterion_BCE(netD(real_reconstruted), label_real)
        errG += reg
        ...
        for j in range(10):
            real_reconstruted = netG(netE(real))
            err_reconstruct = criterion_reconstruct(real, real_reconstruted)
            errE = 2 * lam1 * err_reconstruct - lam2 * criterion_BCE(netD(real_reconstruted), label_real)
