...
with open('./fid_record.txt', 'a') as f:
    f.write("fid_record:" + '\n')
with open('./grad_norm.txt', 'w') as f_grad_norm:
    f_grad_norm.write("grad_norm" + '\n')
...
for epoch in range(nepochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 1000 == 0:
            ...
            with open('./grad_norm.txt', 'a') as f_grad_norm:
                grad_norm = torch.norm(netG.main[3].weight.grad).item()
                f_grad_norm.write(str(grad_norm) + '\n')
    if (epoch + 1) % 10 == 0:
