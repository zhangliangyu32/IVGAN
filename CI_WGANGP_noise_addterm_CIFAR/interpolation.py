...
gamma_D = 0.1 # coefficient for the mutual information
gamma_G = 2
...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    d_interpolates = D(interpolates)[0].view(real_samples.shape[0], 1)
...
for epoch in range(nepochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        errD = -torch.mean(netD(real + sigma * pixd_noise)[0]) + torch.mean(netD(fake.detach())[0]) + lambda_gp * gradient_penalty
        ...
        fake = netG(latent.detach())
        label = torch.full((batch_size,), k, device=device, dtype=torch.int64)
        output = netD(fake)[1]
        CE_regularizer = gamma_D * criterion(output, label)
        errD += CE_regularizer
        ...
        if i % n_critic == 0:
            errG = -torch.mean(netD(netG(noise) + sigma * pixg_noise)[0])
            fake = netG(latent.detach())
            label = torch.full((batch_size,), k, device=device, dtype=torch.int64)
            output = netD(fake)[1]
            CE_regularizer = gamma_G * criterion(output, label)
            errG -= CE_regularizer
        ...
        if i % 1000 == 0:
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f CE_regularizer: %.4f Reconstruct_err: %.4f'
            % (epoch, nepochs, i, len(dataloader), errD.item(), errG.item(), 0 - CE_regularizer.item(), err_reconstruct))
