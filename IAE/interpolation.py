...
for epoch in range(nepochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        pixi_noise = torch.randn(real.size(), device=device)
        ...
        aux_noise1, aux_noise2 = torch.randn(2)
        if aux_noise1.item() < 0:
            ...
            if aux_noise2.item() < 0:
                label = torch.full((batch_size,), np + 1, device=device, dtype=torch.int64)
                output = netD(real)[1]
            else:
                label = torch.full((batch_size,), np + 2, device=device, dtype=torch.int64)
                fake = netG(noise)
                output = netD(fake)[1]
